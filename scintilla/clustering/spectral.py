...
def spectral_clustering(
    ...
    if n_clusters is not None:
        ...
        model = SpectralClustering(
            n_clusters=n_clusters, affinity=affinity, random_state=RANDOM_SEED
        )
    ...
    for nc in n_clusters_range:
        try:
            model = SpectralClustering(
                n_clusters=nc, affinity=affinity, random_state=RANDOM_SEED
            )
            labels = model.fit_predict(X)
            sil = float("nan")
            if len(np.unique(labels)) >= 2:
                try:
                    ...
                    pass
            records.append({"n_clusters": nc, "affinity": affinity, "silhouette": sil})
            if not np.isnan(sil) and sil > best_sil:
                best_sil = sil
                best_labels = labels
        except Exception as e:
            records.append({"n_clusters": nc, "affinity": affinity, "silhouette": float("nan")})
